...
class GlobalExceptionHandlerTest {
    ...
    void handlesSpringSecurityAccessDenied() {
        ResponseEntity<?> response = handler.handleSpringAccessDenied(
    ...
    @Test
    void handlesValidationErrorsWithFieldMessages() throws NoSuchMethodException {
        BeanPropertyBindingResult bindingResult = new BeanPropertyBindingResult(new Object(), "eventRequest");
        bindingResult.addError(new FieldError("eventRequest", "title", "must not be blank"));
        bindingResult.addError(new FieldError("eventRequest", "startDate", "must not be null"));
        ...
        assertThat(bodyMessage(response))
                .isEqualTo("title: must not be blank; startDate: must not be null");
    }
