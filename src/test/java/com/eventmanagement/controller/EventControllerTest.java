...
/**
 * Uses @SpringBootTest rather than the @WebMvcTest slice: the slice doesn't
 * load SecurityConfig's @EnableMethodSecurity, so @PreAuthorize checks would
 * silently pass every request regardless of role.
 */
@SpringBootTest
...
class EventControllerTest {
    ...
    void listEvents_isPublic() throws Exception {
        when(eventService.getAllEvents()).thenReturn(List.of(eventResponse(1L)));
    ...
    void searchEvents_passesFiltersToService() throws Exception {
        when(eventService.searchEvents(eq("jazz"), isNull(), eq("Boston"), isNull(), isNull()))
                .thenReturn(List.of(eventResponse(2L)));
    ...
    @Test
    @WithMockUser(roles = "ORGANIZER")
    void createEvent_blankTitle_isBadRequest() throws Exception {
        EventRequest request = new EventRequest();
        request.setTitle(" ");
        request.setLocation("City Park");
        request.setStartDate(LocalDateTime.of(2026, 6, 1, 18, 0));
    ...
    @Test
    @WithMockUser(roles = "ATTENDEE")
    void updateEvent_asAttendee_isForbidden() throws Exception {
        mockMvc.perform(put("/api/events/1")
                        .contentType("application/json")
                        .content(eventRequestJson()))
                .andExpect(status().isForbidden());

        verify(eventService, never()).updateEvent(anyLong(), any(), any());
    }
